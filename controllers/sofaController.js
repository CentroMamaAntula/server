...
exports.getSOFA = async (req, res) => {
  ...
  try {
    ...
    const sofa = await SOFA.find({ id_paciente: paciente._id })
      .sort({
        date: -1,
      })
      .limit(limit * 1)
    ...
    const count = await SOFA.countDocuments({ id_paciente: id_paciente });
    const totalPages = Math.ceil(count / limit);
    const currentPage = page;
    const total = count;

    res.json({ sofa, totalPages, currentPage, total });
  } catch (e) {
    console.log(e);
